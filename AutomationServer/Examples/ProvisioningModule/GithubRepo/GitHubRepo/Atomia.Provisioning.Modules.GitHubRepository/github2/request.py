...
if not SYSTEM_CERTS and sys.platform.startswith('linux'):
    ...
    '''if we convert githybrepository to exe and call it from automation server then path contains additional subfolders which don't exists; we remove them'''
    try:
        if CA_CERTS.index('Atomia.Provisioning.Modules.GitHubRepository.exe\github2') > 0:
            CA_CERTS = string.replace(CA_CERTS, '\Atomia.Provisioning.Modules.GitHubRepository.exe\github2', '')
    except: pass
